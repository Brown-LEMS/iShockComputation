// By Maruthi Narayanan 01/07/19

#include "dborl/algo/dborl_utilities.h"
#include "esf_params/vox_compute_ishock_esf_params.h"
#include "esf_params/vox_compute_ishock_esf_params_sptr.h"
...
int main(int argc, char *argv[]) {
  ...
  vox_compute_ishock_esf_params_sptr params =
      new vox_compute_ishock_esf_params("dborl_compute_ishock");
  ...
  input_vsol_fn = params->input_object_dir_() + "/" +
                  ...
                  params->input_contour_extension_();
  ...
  if (input_contour_extension == ".cem" || input_contour_extension == ".cemv") {
    ...
    vcl_cerr << "Unknown input type: " << input_contour_extension << " Quit now"
             << vcl_endl;
    return 1;
  }
  ...
  set_process_parameters_of_bpro1(*params, shock_pro,
                                  params->tag_compute_ishock_esf_);
  ...
  if (!ishock_status) {
    ...
    for (; i < 3 * num_iterations; ++i) {
      vcl_cout << vcl_endl;
      vcl_cout << "************ Retry Compute Shock,iter: " << i + 1
               << " *************" << vcl_endl;
  ...
  set_process_parameters_of_bpro1(*params, sample_sg_pro,
  ...
  // Use input from either ishock computation
  ...
  if (sample_shock_results.size() != 1) {
    vcl_cerr << "Sampling of Intrinsinc Shock Computation Failed" << vcl_endl;
    return 1;
  }
  ...
  //  output_file = params->input_object_dir_() + "/"; // original
  ...
  if (!status) {
    vcl_cerr << "Problems in saving extrinsinc shock file: " << output_file
             << vcl_endl;
    return 1;
  }
