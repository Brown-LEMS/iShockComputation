...
dbsk2d_ishock_lineline::dbsk2d_ishock_lineline(
    ...
    _N2L = (-vcl_sin(_ur) * (_Bl.x() - _Ar.x()) +
            vcl_cos(_ur) * (_Bl.y() - _Ar.y())) /
           (1.0 - vcl_cos(_ul - _ur));
    _N2R = (-vcl_sin(_ul) * (_Ar.x() - _Bl.x()) +
            vcl_cos(_ul) * (_Ar.y() - _Bl.y())) /
           (1.0 - vcl_cos(_ur - _ul));
...
double dbsk2d_ishock_lineline::EtaToTau(double eta, DIRECTION dir,
                                        bool constrained) {
    if (dir == LEFT)
...
double dbsk2d_ishock_lineline::rpp(double tau) { return 0; }
...
double dbsk2d_ishock_lineline::k(double tau) { return 0; }

double dbsk2d_ishock_lineline::v(double tau) { return -1 / vcl_cos(phi(tau)); }

double dbsk2d_ishock_lineline::a(double tau) { return 0; }
...
vgl_point_2d<double> dbsk2d_ishock_lineline::getPtFromLTau(double tau) {
    ...
    // Here we compute the vgl_point_2d<double> directly, without reference to
    // _origin
