...
class dbsk2d_ishock_contact : public dbsk2d_ishock_edge {
  protected:
    ...
    double _RsEta; ///< eta parameter of the right element
                   // these have to be stored because they cannot be inferred
                   // from the taus
  public:
    //: Constructor
    dbsk2d_ishock_contact(int newid, dbsk2d_ishock_belm* lbe,
                          dbsk2d_ishock_belm* rbe, vgl_point_2d<double> origin,
                          double n, double lstau, double rstau, double lseta,
                          double rseta);
    //: destructor
    ...
    virtual double EtaToTau(double eta, DIRECTION dir,
    ...
    virtual vgl_point_2d<double> getEndPt(void) {
        return _translatePoint(_origin, _n, _endTime);
    }
    virtual vgl_point_2d<double> getMidPt(void) {
        return _translatePoint(_origin, _n,
                               vnl_math::min(_endTime, MAX_RADIUS) / 2);
    }
    virtual vgl_point_2d<double> getEndPtWithinRange(void) {
        return _translatePoint(_origin, _n,
                               vnl_math::min(_endTime, MAX_RADIUS));
    }
