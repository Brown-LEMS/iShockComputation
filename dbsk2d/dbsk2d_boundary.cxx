...
// -----------------------------------------------------------
//: Constructor
dbsk2d_boundary::dbsk2d_boundary()
    : _nextAvailableID(0), ishock_graph_(0), shock_graph_(0), cell_h_(-1),
      cell_w_(-1), xmin_(0), ymin_(0) {
...
void dbsk2d_boundary::all_contours(bnd_contour_list& all_contours) const {
...
void dbsk2d_boundary::compute_bounding_box() const {
...
void dbsk2d_boundary::set_partition_params(double xmin, double ymin,
    ...
    for (int i = 0; i < num_rows; ++i) {
        for (int j = 0; j < num_cols; ++j) {
            ...
            this->cells_(i, j) = new dbsk2d_bnd_cell(
                dbsk2d_bnd_cell_index(i, j),
...
void dbsk2d_boundary::partition_into_cells(bool tight, bool talkative,
                                           double tol) {
...
void dbsk2d_boundary::insert_edge_into_cells_using_bounding_box(
    const dbsk2d_bnd_edge_sptr& e, double tol) {
...
void dbsk2d_boundary::insert_line_into_cells_tight(
    const dbsk2d_bnd_edge_sptr& e, double tol) {
    ...
    for (vcl_list<dbsk2d_bnd_cell_sptr>::iterator cit = cells_line1.begin();
         cit != cells_line1.end(); ++cit) {
        (*cit)->add_bnd_edge(e);
    }
}

//: Compute list of cells that a point belongs to (fuzzily)
vcl_vector<dbsk2d_bnd_cell_sptr>
dbsk2d_boundary::compute_cell_membership_of_point(
    const vgl_point_2d<double>& pt, double tol) const {
    vcl_vector<dbsk2d_bnd_cell_sptr> cell_list;
    cell_list.clear();

    // compute range of boxes this edge belongs to.
    int min_col = static_cast<int>(
        vcl_floor((pt.x() - tol - this->xmin()) / this->cell_w()));
    int max_col = static_cast<int>(
        vcl_floor((pt.x() + tol - this->xmin()) / this->cell_w()));
    int min_row = static_cast<int>(
        vcl_floor((pt.y() - tol - this->ymin()) / this->cell_h()));
...
void dbsk2d_boundary::compute_cell_membership_of_line(
    const vgl_point_2d<double>& p1, const vgl_point_2d<double>& p2,
    ...
    for (unsigned int i = 0; i < midpoints.size(); ++i) {
        int col =
            (int)vcl_floor((midpoints[i].x() - this->xmin()) / this->cell_w());
        int row =
            (int)vcl_floor((midpoints[i].y() - this->ymin()) / this->cell_h());
        // ignore if `col' and `row' are out of range
        if (col < 0 || col >= this->num_cols() || row < 0 ||
            row >= this->num_rows())
            continue;
...
vcl_vector<dbsk2d_bnd_cell_sptr>
dbsk2d_boundary::compute_cell_membership_of_bbox(const vgl_box_2d<double>& bbox,
                                                 double tol) const {
    vcl_vector<dbsk2d_bnd_cell_sptr> ret_cells;

    // display an error when edge is outside partitioned area
    bool inside_partition_area =
        (bbox.min_x() >= this->xmin() &&
    ...
    // compute range of boxes this edge belongs to.
    int min_col = static_cast<int>(
        vcl_floor((box.min_x() - this->xmin()) / this->cell_w()));
    int max_col = static_cast<int>(
        vcl_floor((box.max_x() - this->xmin()) / this->cell_w()));
    int min_row = static_cast<int>(
        vcl_floor((box.min_y() - this->ymin()) / this->cell_h()));
    int max_row = static_cast<int>(
        vcl_floor((box.max_y() - this->ymin()) / this->cell_h()));
...
void dbsk2d_boundary::intersect_line_with_cell_grids(
    const vgl_point_2d<double>& p1, const vgl_point_2d<double>& p2,
    ...
    int start_col =
        (int)(vcl_ceil((box.min_x() - this->xmin()) / this->cell_w()));
    int end_col =
        (int)(vcl_ceil((box.max_x() - this->xmin()) / this->cell_w())) - 1;

    int start_row =
        (int)(vcl_ceil((box.min_y() - this->ymin()) / this->cell_h()));
    int end_row =
        (int)(vcl_ceil((box.max_y() - this->ymin()) / this->cell_h())) - 1;

    for (int m = start_col; m <= end_col; ++m) {
        // That we are here means the linesegment is not a vertical line
        intersections.push_back(
            vnl_math::abs(this->xmin() + m * this->cell_w() - p1.x()) /
            box.width());
    }
    ...
    for (int m = start_row; m <= end_row; ++m) {
        ...
        intersections.push_back(
            vnl_math::abs(this->ymin() + m * this->cell_h() - p1.y()) /
            box.height());
    }
    intersections.sort();
    return;
...
// Return a sorted list of parameters of intersections points
void dbsk2d_boundary::intersect_arc_with_cell_grids(
    const vgl_point_2d<double>& arc_p1, const vgl_point_2d<double>& arc_p2,
    double arc_k, vcl_list<double>& intersections) const {
    ...
    // compute the cells this bbox belongs to, which are potential containers of
    // the arc
    vcl_vector<dbsk2d_bnd_cell_sptr> cells =
        this->compute_cell_membership_of_bbox(bbox, h);

    // for each cell, check whether the arc ACTUALLY intersects it.
    // This is not the most effecient way to do it but it is an easy and safe
    // way to do it
    for (unsigned i = 0; i < cells.size(); ++i) {
        ...
        for (unsigned i = 0; i < line_list.size(); ++i) {
            ...
            double d = dbgl_closest_point::lineseg_to_circular_arc(
                line_list[i], arc, line_ratios, arc_ratios);
...
void dbsk2d_boundary::break_long_line_edges(double tol) {
    ...
    for (bnd_edge_list::iterator eit = all_edges.begin();
        ...
        // if the bbox spans over two cells, that means the point is close to a
        // boundary
        if (this->compute_cell_membership_of_point(e->point1(), tol).size() >
            1) {
        ...
        for (unsigned int i = 0; i < vertices_along_edge.size(); ++i) {
            ...
            dbsk2d_ishock_bline* left = new dbsk2d_ishock_bline(
                bp1, bp2, this->nextAvailableID(), true);
            dbsk2d_ishock_bline* right = new dbsk2d_ishock_bline(
                bp2, bp1, this->nextAvailableID(), false);
        ...
        while (e->numsup() > 0) {
            ...
            dbsk2d_bnd_contour_sptr contour =
                static_cast<dbsk2d_bnd_contour*>(t);
            ...
            if (dir == 1) {
                ...
                // only reverse copy from `new_edges' to `reverse_new_edges'
                // once
                ...
                contour->replace_edges(reverse_new_edges, reverse_directions,
                                       e);
            }
        }
        ...
        // We shall use midpoints to determine which cell a linesegment belongs
        // to
        ...
        for (unsigned int i = 0; i < new_edges.size(); ++i) {
            ...
            vgl_point_2d<double> p =
                centre<double>(new_e->point1(), new_e->point2());

            // compute range of boxes this edge belongs to.
            int min_col = static_cast<int>(
                vcl_floor((p.x() - tol - this->xmin()) / this->cell_w()));
            int max_col = static_cast<int>(
                vcl_floor((p.x() + tol - this->xmin()) / this->cell_w()));
            int min_row = static_cast<int>(
                vcl_floor((p.y() - tol - this->ymin()) / this->cell_h()));
            int max_row = static_cast<int>(
                vcl_floor((p.y() + tol - this->ymin()) / this->cell_h()));
...
// extend beyond one cell (fuzzily)
// Required: edges have been put in the cells (use partition_into_cells())
void dbsk2d_boundary::break_long_arc_edges(double tol) {
    // collect all the edges of `this' boundary
    bnd_edge_list all_edges;
    dbsk2d_bnd_utils::extract_edge_list(this, all_edges);
    for (bnd_edge_list::iterator eit = all_edges.begin();
        ...
        dbsk2d_ishock_barc* left_barc =
            static_cast<dbsk2d_ishock_barc*>(e->left_bcurve());
        dbsk2d_ishock_barc* right_barc =
            static_cast<dbsk2d_ishock_barc*>(e->right_bcurve());

        dbgl_circ_arc arc(left_barc->start(), left_barc->end(),
                          left_barc->curvature());

        // intersection locations
        vcl_list<double> intersections;
        this->intersect_arc_with_cell_grids(arc.start(), arc.end(), arc.k(),
                                            intersections);
        ...
        for (vcl_list<double>::iterator it = intersections.begin();
             it != intersections.end(); ++it) {
            vertices_along_edge.push_back(
                dbsk2d_bnd_utils::new_vertex(arc.point_at(*it), this));
        }
        ...
        // if the bbox spans over two cells, that means the point is close to a
        // boundary
        if (this->compute_cell_membership_of_point(e->point2(), tol).size() >
            1) {
            dbsk2d_bnd_vertex_sptr v = e->bnd_v2();
        ...
        for (unsigned int i = 0; i < vertices_along_edge.size(); ++i) {
            ...
            // create left and right blines
            dbsk2d_ishock_barc* left = new dbsk2d_ishock_barc(
                bp1, bp2, this->nextAvailableID(), true, left_barc->center(),
                left_barc->R(), left_barc->nud());
            dbsk2d_ishock_barc* right = new dbsk2d_ishock_barc(
                bp2, bp1, this->nextAvailableID(), false, right_barc->center(),
...
void dbsk2d_boundary::clear_cells() {
...
void dbsk2d_boundary::update_belm_list(
    ...
    dbsk2d_bnd_vertex_sptr endpt =
        new_contour->bnd_vertex(new_contour->num_edges());
...
dbsk2d_boundary::add_a_point(const vgl_point_2d<double>& pt,
                             bool preproc_needed) {
...
//: Add a stand-alone point, return smart ptr to the new dbsk2d_bnd_contour
dbsk2d_bnd_contour_sptr
dbsk2d_boundary::add_a_point(const vsol_point_2d_sptr& point,
                             bool preproc_needed) {
...
dbsk2d_bnd_contour_sptr
dbsk2d_boundary::add_a_line(const vgl_point_2d<double>& p1,
...
//: Add a line segment, return smart ptr to the new the dbsk2d_bnd_contour
dbsk2d_bnd_contour_sptr
dbsk2d_boundary::add_a_line(const vsol_line_2d_sptr& line,
                            bool preproc_needed) {
...
dbsk2d_bnd_contour_sptr
dbsk2d_boundary::add_a_polyline(const vsol_polyline_2d_sptr& polyline,
                                bool preproc_needed) {
    ...
    for (unsigned int i = 0; i < polyline->size(); i++) {
        ...
        // out << polyline->vertex(i)->get_p().x() << " " <<
        // polyline->vertex(i)->get_p().y() << vcl_endl;
    }
    //	out << vcl_endl;
    //	out.close();
...
dbsk2d_bnd_contour_sptr
dbsk2d_boundary::add_a_polygon(const vsol_polygon_2d_sptr& polygon,
                               bool preproc_needed) {
...
dbsk2d_bnd_contour_sptr
dbsk2d_boundary::add_an_interp_curve_2d(const dbsol_interp_curve_2d_sptr& curve,
                                        bool preproc_needed) {
    // check existence
    if (!curve)
        return 0;
...
dbsk2d_bnd_contour_sptr dbsk2d_boundary::add_connected_lines(
...
dbsk2d_bnd_contour_sptr dbsk2d_boundary::add_an_arc(vgl_point_2d<double> arc_p1,
                                                    vgl_point_2d<double> arc_p2,
                                                    double arc_k,
                                                    bool preproc_needed) {
    vcl_vector<vgl_point_2d<double>> pts;
...
dbsk2d_bnd_contour_sptr dbsk2d_boundary::add_connected_arcs(
    const vcl_vector<vgl_point_2d<double>>& vertices,
    const vcl_vector<double>& curvatures, bool closed, bool preproc_needed) {
    if (vertices.size() < 2)
        return 0;
    ...
    if (closed)
        dbsk2d_assert(curvatures.size() >= (vertices.size()));
    ...
    for (unsigned int i = 0; i < bv_list.size() - 1; ++i) {
        dbgl_circ_arc arc(bv_list[i]->point(), bv_list[i + 1]->point(),
                          curvatures[i]);
        ...
        if (arc.height() < B_EPSILON) {
            bnd_edges.push_back(dbsk2d_bnd_utils::new_line_between(
                bv_list[i], bv_list[i + 1], this));
        } else {
            bnd_edges.push_back(dbsk2d_bnd_utils::new_arc_between(
                bv_list[i], bv_list[i + 1], curvatures[i], this));
        }
    }
...
void dbsk2d_boundary::remove_empty_contours() {
...
void dbsk2d_boundary::add_a_bnd_contour(
    const dbsk2d_bnd_contour_sptr& new_bnd_contour, bool preproc_needed) {
    if (preproc_needed) {
...
// Need rewrite.
void dbsk2d_boundary::print(vcl_ostream& os) const {
    bnd_contour_list all_contours;
    this->all_contours(all_contours);
...
//: describe
// Need rewrite.
void dbsk2d_boundary::describe(vcl_ostream& os, int blanking) const {
    for (int j = 0; j < blanking; ++j)
        os << ' ';
    this->print(os);
    bnd_contour_list all;
    this->all_contours(all);
    for (bnd_contour_list::iterator cit = all.begin(); cit != all.end();
         ++cit) {
...
// -----------------------------------------------------------------------
//: Print the belm list
void dbsk2d_boundary::print_belm_list(vcl_ostream& os) {
...
void dbsk2d_boundary::print_partition_summary(vcl_ostream& os) const {
    os << "Partition params: \n"
       << "(xmin, ymin)= (" << this->xmin() << "," << this->ymin() << ")\n"
       << "num_rows= " << this->num_rows()
       << "   num_cols= " << this->num_cols()
       << "\ncell_height= " << this->cell_h()
       << "  cell_width= " << this->cell_w() << "\n(xmax, ymax)=("
       << this->xmin() + this->width() << "," << this->ymin() + this->height()
       << ") \n";
}

// dbsk2d_ishock_bpoint* dbsk2d_boundary::
// chopBElement(dbsk2d_ishock_belm* belm, dbsk2d_ishock_bpoint* StartPt,
// vgl_point_2d<double> EndPt, bool chopCompletely)
//{
//   //: \todo too much of this function needed to be copied to handle shocks.
//   Reiterate
//
...
//
//         if (bchopEnd)
//           add_an_arc_segment_between(start_pt, end_pt, cen, R, nud,
//           ARC_NUS_SMALL);
//         else
//           add_an_arc_segment_between(end_pt, start_pt, cen, R, nud,
//           ARC_NUS_SMALL);
//       }
//       break;
//     }
...
//
//   //finally delete them
//   for (ishock_elm_list_iter curC = ContactsToDel.begin();
//   curC!=ContactsToDel.end(); ++curC)
//   {
...
//       if (bline->twinLine()){
//         belm_list neighboringBElms =
//         bline->twinLine()->getAllNeighboringBElements();
//         AllNeighboringBElms.merge(neighboringBElms);
//         neighboringBElms.clear();
//       }
//
...
//// following the deletion of the elements. On second thoughts, maybe I decided
/// that / this was not a safe thing to do. Check it out. }
...
// add_a_line_segment_between (dbsk2d_ishock_bpoint* spt, dbsk2d_ishock_bpoint*
// ept)
//{
...
//         k = 1/getArcRadiusFromThreePoints(lLine->e_pt()->pt(), spt->pt(),
//         ept->pt());
//       }
//       else {
//         dtheta = vcl_fabs(u- lLine->u());
//         l = _distPointPoint(spt->pt(),lLine->s_pt()->pt());
//         k = 1/getArcRadiusFromThreePoints(lLine->s_pt()->pt(), spt->pt(),
//         ept->pt());
//       }
//
//       //Trying to fit the lines as arc in a box smaller than the accuracy of
//       the input wL = l*l*k/8;
//     }
//   }
//
...
//         k = 1/getArcRadiusFromThreePoints(rLine->e_pt()->pt(), ept->pt(),
//         spt->pt());
//       }
//       else {
//         dtheta = vcl_fabs(u- angle0To2Pi(rLine->u()+vnl_math::pi));
//         l = _distPointPoint(ept->pt(),rLine->s_pt()->pt());
//         k = 1/getArcRadiusFromThreePoints(rLine->s_pt()->pt(), ept->pt(),
//         spt->pt());
//       }
//
//       //Trying to fit the lines as arc in a box smaller than the accuracy of
//       the input wR = l*l*k/8;
//     }
//   }
//
...
//   }
//
//   //Re-activate existing dbsk2d_ishock_contact to removes the chance of zero
//   length shocks if (!fin_spt->is_a_free_point()){
//     bnd_ishock_map_iter curS = fin_spt->shock_map().begin();
//     for (; curS!=fin_spt->shock_map().end(); ++curS) {
//       if (curS->second->label() == dbsk2d_ishock_elm::CONTACT) {
//         dbsk2d_ishock_contact* contact =
//         (dbsk2d_ishock_contact*)curS->second; _shock->remove_vertex
//         (contact->cSNode());
//       }
//     }
//   }
...
//
//   dbsk2d_ishock_bline* newelm = new dbsk2d_ishock_bline (fin_spt, fin_ept,
//   nextAvailableID(), true); //GUI one dbsk2d_ishock_bline* twinelm = new
//   dbsk2d_ishock_bline (fin_ept, fin_spt, nextAvailableID(), false); //Non-GUI
//
...
//     return NULL;
//
//   dbsk2d_ishock_bpoint* spt = new dbsk2d_ishock_bpoint (sx, sy,
//   nextAvailableID()); dbsk2d_ishock_bpoint* ept = new dbsk2d_ishock_bpoint
//   (ex, ey, nextAvailableID()); addBElement (spt);
//    addBElement (ept);
//
//   dbsk2d_ishock_belm* newelm = add_an_arc_segment_between (spt, ept,
//   vgl_point_2d<double>(cx,cy), r, nud, nus);
//
//    return newelm;
// }
//
//
// dbsk2d_ishock_belm* dbsk2d_boundary::
// add_an_arc_segment_between (dbsk2d_ishock_bpoint* spt, dbsk2d_ishock_bpoint*
// ept, vgl_point_2d<double> center,
//                           double r, ARC_NUD nud, ARC_NUS nus)
//{
//   //if the two boundary points are close, delete one point
//   if (_BisEqPoint(spt->pt(), ept->pt())){
...
//
//     //add the midpoint
//     dbsk2d_ishock_bpoint* mpt = new dbsk2d_ishock_bpoint (mpoint.x(),
//     mpoint.y(), nextAvailableID()); addBElement (mpt);
//
//     add_an_arc_segment_between (mpt, ept, center, r, nud, ARC_NUS_SMALL);
//     return add_an_arc_segment_between (spt, mpt, center, r, nud,
//     ARC_NUS_SMALL);
//   }
//   else { //2)Small arc case:
//     dbsk2d_ishock_barc* newelm = new dbsk2d_ishock_barc (spt, ept,
//     nextAvailableID(), true, center, r, nud); dbsk2d_ishock_barc* twinelm =
//     new dbsk2d_ishock_barc (ept, spt, nextAvailableID(), false, center, r,
//     (ARC_NUD)(-(int)nud));
//
//     //link to one another (best to do this here)
...
//
// dbsk2d_ishock_belm* dbsk2d_boundary::
// add_an_arc_segment_between (vgl_point_2d<double> spt, dbsk2d_ishock_bpoint*
// bept, vgl_point_2d<double> center,
//                        double r, ARC_NUD nud, ARC_NUS nus)
//{
...
//   }
//
//   dbsk2d_ishock_bpoint* bspt = new dbsk2d_ishock_bpoint (spt.x(), spt.y(),
//   nextAvailableID()); addBElement (bspt);
//
...
//
// dbsk2d_ishock_belm* dbsk2d_boundary::
// add_an_arc_segment_between (dbsk2d_ishock_bpoint* bspt, vgl_point_2d<double>
// ept, vgl_point_2d<double> center,
//                        double r, ARC_NUD nud, ARC_NUS nus)
//{
...
//   }
//
//   dbsk2d_ishock_bpoint* bept = new dbsk2d_ishock_bpoint (ept.x(), ept.y(),
//   nextAvailableID()); addBElement (bept);
//
//   return add_an_arc_segment_between (bspt, bept, center, r, nud, nus);
// }
//
//
// void dbsk2d_boundary::
// add_biarc_segments_between (dbsk2d_ishock_bpoint* arc_start_pt, double
// SAngle,
...
//   //BiArcShock new_biArc(arc_start_pt->pt(), SAngle, arc_end_pt->pt(),
//   EAngle);
...
//
//   //dbsk2d_ishock_bpoint* mid_pt = (dbsk2d_ishock_bpoint*)
//   add_a_point(mid.x(), mid.y());
//   //vgl_point_2d<double> cen1 = getCenterOfArc (arc_start_pt->pt().x(),
//   arc_start_pt->pt().y(), mid_pt->pt().x(), mid_pt->pt().y(), R1, (ARC_NUD)
//   dir1, ARC_NUS_SMALL);
//   //vgl_point_2d<double> cen2 = getCenterOfArc (mid_pt->pt().x(),
//   mid_pt->pt().y(), arc_end_pt->pt().x(), arc_end_pt->pt().y(), R2, (ARC_NUD)
//   dir2, ARC_NUS_SMALL);
//   //add_an_arc_segment_between (arc_start_pt, mid_pt, cen1, R1, (ARC_NUD)
//   dir1, ARC_NUS_SMALL);
//   //add_an_arc_segment_between (mid_pt, arc_end_pt, cen2, R2, (ARC_NUD) dir2,
//   ARC_NUS_SMALL);
// }
