...
dbsk2d_ishock_pointarc::dbsk2d_ishock_pointarc(int newid, double stime,
...
double dbsk2d_ishock_pointarc::computeMaxLTau() {
    if (_s == 1) {
        if (_nu == 1) { // Case 1:
            ...
            if (AisLEq(letau,
                       _LsTau)) // tau corresponding to min_eta is out of range
                letau = 2 * vnl_math::pi;
...
double dbsk2d_ishock_pointarc::computeMinRTau() {
    if (_s == 1) {
        if (_nu == 1) { // Case 1:
            ...
            if (AisGEq(retau,
                       _RsTau)) // tau corresponding to max_eta is out of range
                retau = 0;
...
//: correct intrinsic parameters at the start of the shock (so that the left and
//: right agree)
...
double dbsk2d_ishock_pointarc::EtaToTau(double eta, DIRECTION dir,
                                        bool constrained) {
    if (dir == LEFT)
...
double dbsk2d_ishock_pointarc::REtaToRTau(double eta, bool constrained) {
    ...
    if (constrained)
        dbsk2d_assert(
            AisBetween(rtau, 3 * vnl_math::pi_over_2, 2 * vnl_math::pi));
...
double dbsk2d_ishock_pointarc::LTauToLEta(double tau, bool start) {
    ...
    if (_nu > 0) // point on the left
    {
        leta = this->lBPoint()->vec_to_eta(_u + tau); // range
                                                      // [min_eta..max_eta]

        // correct eta at the ends (point etas can have 0-2pi discontinuity
        // issues)
...
double dbsk2d_ishock_pointarc::RTauToREta(double tau, bool start) {
    ...
    if (_nu > 0) // arc on the right
    {
        ...
        reta = this->rBPoint()->vec_to_eta(_u + vnl_math::pi +
                                           tau); // range [min_eta..max_eta]
...
double dbsk2d_ishock_pointarc::rp(double tau) { return 0; }

double dbsk2d_ishock_pointarc::rpp(double tau) { return 0; }

double dbsk2d_ishock_pointarc::g(double tau) { return 0; }
...
double dbsk2d_ishock_pointarc::k(double tau) { return 0; }
...
double dbsk2d_ishock_pointarc::a(double tau) { return 0; }

double dbsk2d_ishock_pointarc::phi(double tau) { return 0; }
...
dbsk2d_ishock_edge::TAU_DIRECTION_TYPE dbsk2d_ishock_pointarc::tauDir() {
    if (_s == -1 && _nu == -1)
...
vgl_point_2d<double> dbsk2d_ishock_pointarc::getLFootPt(double ltau) {
    if (_nu == 1)
        ...
        return _translatePoint(((dbsk2d_ishock_barc*)_lBElement)->center(),
                               angle0To2Pi(_u + ltau),
                               ((dbsk2d_ishock_barc*)_lBElement)->R());
}
...
vgl_point_2d<double> dbsk2d_ishock_pointarc::getRFootPt(double tau) {
    ...
    if (_nu == -1)
        ...
        return _translatePoint(((dbsk2d_ishock_barc*)_rBElement)->center(),
                               angle0To2Pi(_u + vnl_math::pi + rtau),
                               ((dbsk2d_ishock_barc*)_rBElement)->R());
}
