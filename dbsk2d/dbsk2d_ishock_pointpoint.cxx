...
dbsk2d_ishock_pointpoint::dbsk2d_ishock_pointpoint(
...
// when constrained, the taus returned by this function needs to be shifted to
// the valid range else return the raw result
double dbsk2d_ishock_pointpoint::EtaToTau(double eta, DIRECTION dir,
                                          bool constrained) {
    if (dir == LEFT)
...
double dbsk2d_ishock_pointpoint::RTauToREta(double tau, bool start) {
    ...
    double reta = this->rBPoint()->vec_to_eta(_u + vnl_math::pi +
...
double dbsk2d_ishock_pointpoint::rpp(double tau) {
    return _H / (vcl_cos(tau) * vcl_cos(tau) * vcl_cos(tau)) -
           _H / (2 * vcl_cos(tau));
}
...
double dbsk2d_ishock_pointpoint::k(double tau) { return 0; }
...
double dbsk2d_ishock_pointpoint::a(double tau) { return 0; }
...
vgl_point_2d<double> dbsk2d_ishock_pointpoint::getPtFromLTau(double ltau) {
    ...
    pt = _origin +
         _rotateCCW(vgl_vector_2d<double>(d * vcl_cos(ltau), d * vcl_sin(ltau)),
                    _u);
