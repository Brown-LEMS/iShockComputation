...
inline VECTOR_TYPE _vPointPoint(vgl_point_2d<double> start,
                                vgl_point_2d<double> end) {
    return angle0To2Pi(vcl_atan2(end.y() - start.y(), end.x() - start.x()));
}
...
//: compute the new position of a point given the displacement direction and
//: length
inline vgl_point_2d<double> _translatePoint(vgl_point_2d<double> pt,
                                            VECTOR_TYPE v, double length) {
...
inline double _distPointPoint(vgl_point_2d<double> p1,
                              vgl_point_2d<double> p2) {
    return vnl_math::hypot((p2.x() - p1.x()), (p2.y() - p1.y()));
}

//: square of the distance between two points
inline double _distSqPointPoint(vgl_point_2d<double> p1,
                                vgl_point_2d<double> p2) {
...
inline vgl_point_2d<double> _midPointPoint(vgl_point_2d<double> p1,
                                           vgl_point_2d<double> p2) {
...
inline int _CCW(vgl_point_2d<double>& p0, vgl_point_2d<double>& p1,
                vgl_point_2d<double>& p2) {
...
inline bool _areLinesIntersecting(vgl_point_2d<double>& line1s,
                                  vgl_point_2d<double>& line1e,
                                  vgl_point_2d<double>& line2s,
                                  vgl_point_2d<double>& line2e) {
    return ((_CCW(line1s, line1e, line2s) * _CCW(line1s, line1e, line2e)) <=
            0) &&
...
inline double _distPointLine(vgl_point_2d<double> pt,
...
inline double _distSqPointLine(vgl_point_2d<double> pt,
                               vgl_point_2d<double> lstart,
                               vgl_point_2d<double> lend) {
    // Ken: divide is slow.
...
inline vgl_point_2d<double> _getFootPt(vgl_point_2d<double> pt,
                                       vgl_point_2d<double> lstart,
                                       vgl_point_2d<double> lend) {
...
inline double _deltaPointLine(vgl_point_2d<double> pt,
                              vgl_point_2d<double> lstart,
                              vgl_point_2d<double> lend, double line_length) {
    // Ken: divide is slow.
    return ((pt.y() - lstart.y()) * (lend.y() - lstart.y()) +
            (pt.x() - lstart.x()) * (lend.x() - lstart.x())) /
           line_length;
...
inline bool _isPointAboveLine(vgl_point_2d<double> pt,
                              vgl_point_2d<double> lstart,
                              vgl_point_2d<double> lend) {
    return ((pt.x() - lstart.x()) * (lstart.y() - lend.y()) +
            (pt.y() - lstart.y()) * (lend.x() - lstart.x())) > 0;
}
...
inline bool _isPointInsideRect(vgl_point_2d<double> pt, double l, double t,
                               double r, double b) {
    // Ken:
...
enum ARC_NUD { BOGUS_ARC_NUD = 0, ARC_NUD_CCW = -1, ARC_NUD_CW = +1 };

enum ARC_NUS { BOGUS_ARC_NUS = 0, ARC_NUS_LARGE = -1, ARC_NUS_SMALL = +1 };

//: get the center point of an arc
inline vgl_point_2d<double> getCenterOfArc(double sx, double sy, double ex,
                                           double ey, double r, ARC_NUD nud,
                                           ARC_NUS nus) {
    int nu = 0; // not possible just a precaution
    double tau;
    double d = vcl_sqrt((ey - sy) * (ey - sy) + (ex - sx) * (ex - sx));

    if (_isEq(d, 2 * r, R_EPSILON))
        tau = 0;
    else
        tau = vcl_acos(d / (2 * r));
    ...
    return vgl_point_2d<double>(
        (sx + ex) / 2 + (d / 2) * vcl_tan(nu * tau) * vcl_cos(psi),
...
inline double getArcRadiusFromThreePoints(double x1, double y1, double x2,
...
// Pt1-----Pt2----Pt3
inline double getArcRadiusFromThreePoints(vgl_point_2d<double> Pt1,
                                          vgl_point_2d<double> Pt2,
                                          vgl_point_2d<double> Pt3) {
    return getArcRadiusFromThreePoints(Pt1.x(), Pt1.y(), Pt3.x(), Pt3.y(),
                                       Pt2.x(), Pt2.y());
}
...
inline double getTangentFromThreePoints(double x1, double y1, double x2,
                                        double y2, double x3, double y3) {
    double vcl_tan_chord = angle0To2Pi(vcl_atan2(y2 - y1, x2 - x1));
    vgl_point_2d<double> center =
        getArcCenterFromThreePoints(x1, y1, x2, y2, x3, y3);

    if (center.x() == 100000)
        return vcl_tan_chord;

    double tangent1 = angle0To2Pi(vcl_atan2(center.y() - y3, center.x() - x3) +
                                  vnl_math::pi_over_2);
    double tangent2 = angle0To2Pi(vcl_atan2(center.y() - y3, center.x() - x3) -
                                  vnl_math::pi_over_2);
    double tangent;
...
inline vgl_point_2d<double> centerOfArc(const vgl_point_2d<double>& point1,
                                        ...
                                        ARC_NUS& nus) {
    double chordVector =
        vcl_atan2(point2.y() - point1.y(), point2.x() - point1.x());
    vgl_point_2d<double> midChord((point2.x() + point1.x()) / 2,
                                  (point2.y() + point1.y()) / 2);

    double chordL =
        vnl_math::hypot(point2.y() - point1.y(), point2.x() - point1.x());
    double d = vcl_sqrt(radius * radius - chordL * chordL / 4);

    double direction = chordVector;
    if (nud == ARC_NUD_CCW)
        if (nus == ARC_NUS_SMALL)
            direction += vnl_math::pi / 2;
        else
            direction -= vnl_math::pi / 2;
    else if (nus == ARC_NUS_SMALL)
        direction -= vnl_math::pi / 2;
    else
        direction += vnl_math::pi / 2;
...
inline bool threePointsToArc(const vgl_point_2d<double>& p1,
                             const vgl_point_2d<double>& p2,
                             const vgl_point_2d<double>& p3,
                             vgl_point_2d<double>& center, double& radius,
                             ARC_NUD& nud) {
    double A = p2.x() - p1.x();
    double B = p2.y() - p1.y();
    double C = p3.x() - p1.x();
    ...
    vgl_point_2d<double> &center, double &radius, ARC_NUD &nud)
{
  ...
  double
    psi1 = vcl_atan2(point2.y()-point1.y(),point2.x()-point1.x()) +
vnl_math::pi/2, psi2 = vcl_atan2(point2.y()-point3.y(),point2.x()-point3.x()) +
vnl_math::pi/2;
...
inline bool threePointsToArc(const vgl_point_2d<double>& point1,
                             ...
                             vgl_point_2d<double>& center, double& radius,
                             double& theta_first, double& theta_second) {
    double x1 = point1.x(), x2 = point2.x(), x3 = point3.x(), y1 = point1.y(),
           y2 = point2.y(), y3 = point3.y(), l1 = x1 * x1 + y1 * y1,
           l2 = x2 * x2 + y2 * y2, l3 = x3 * x3 + y3 * y3;
    ...
    f = -(y1 * (l2 * x3 - x2 * l3) - y2 * (l1 * x3 - l3 * x1) +
          y3 * (l1 * x2 - x1 * l2));
...
inline void pointTangentPointToArc(const vgl_point_2d<double>& point1,
                                   double theta,
                                   const vgl_point_2d<double>& point2,
                                   vgl_point_2d<double>& center, double& radius,
                                   ARC_NUD& nud) {
...
inline double getTangentOfArc(const vgl_point_2d<double>& center,
                              ...
                              const vgl_point_2d<double>& end, ARC_NUD nud) {
...
inline double getTangentOfArc(const vgl_point_2d<double>& center, double radius,
                              double theta_first, double theta_second,
                              const vgl_point_2d<double>& start,
...
////: Return a point on a line segment, specified by the ratio between its
///length to the starting point / and the length of the line segment. / Note if
///ratio < 0 or ratio > 1 then the point lies outside the line segment
// inline vgl_point_2d<double > point_on_line_segment(double sx, double sy,
// double ex, double ey, double ratio)
//{
//   return vgl_point_2d<double >(ex*ratio + sx*(1-ratio), ey*ratio +
//   sy*(1-ratio));
// };
//
// inline vgl_point_2d<double > point_on_line_segment(const
// vgl_line_segment_2d<double >& line, double ratio)
//{
//   return point_on_line_segment(line.point1().x(), line.point1().y(),
//   line.point2().x(), line.point2().y(), ratio);
// };
