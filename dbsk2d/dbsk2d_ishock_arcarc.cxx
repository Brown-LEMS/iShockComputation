...
dbsk2d_ishock_arcarc::dbsk2d_ishock_arcarc(int newid, double stime,
    ...
    if (_MU == 1) {
        ...
        if (_s == 1) { // Hyperbola by intersecting circles
            _emu = 0;  // not applicable
            if (lBArc()->nud() == ARC_NUD_CW && rBArc()->nud() == ARC_NUD_CW)
                _hmu = 1;
            else
                _hmu = -1;
        } else {      // Ellipse inside intersecting circles
            _hmu = 0; // not applicable
            if (lBArc()->nud() == ARC_NUD_CW && rBArc()->nud() == ARC_NUD_CCW)
                _emu = 1;
            else
                _emu = -1;
        }
    ...
    if (_MU == 1) {
        if (_s == 1) // Case 1, Case 2:
            ...
            return AisGEq(_LsTau, vnl_math::pi) &&
                   AisLEq(_LsTau, 2 * vnl_math::pi);
    } else { //_MU=-1
        if (_s == 1) {
            if (_hmu == 1) // Case 5, Case 6:
                return AisGEq(_LsTau, 0) &&
                       AisLEq(_LsTau,
                              _LAsymTau); // should be AisGEq(_LsTau,_LintTau)
            else                          // Case 7, Case 8:
                return AisGEq(_LsTau, 0) &&
                       AisLEq(
                           _LsTau,
            ...
            if (_emu == 1) // Case 9, Case 10
                return AisGEq(_LsTau, vnl_math::pi) &&
                       AisLEq(_LsTau,
                              2 * vnl_math::pi); // should be
                                                 // AisGEq(_LsTau,_LintTau)
            else                                 // Case 11, Case 12:
                return AisGEq(_LsTau, vnl_math::pi) &&
                       AisLEq(_LsTau,
                              2 * vnl_math::pi); // should be
                                                 // AisLEq(_LsTau,_LintTau)
        }
    }
}
...
bool dbsk2d_ishock_arcarc::isRSTauValid() {
    if (_MU == 1) {
        if (_s == 1) // Case 1, Case 2:
            return AisGEq(_RsTau, _RAsymTau) &&
                   AisLEq(_RsTau, 2 * vnl_math::pi);
        else // Case 3, Case 4:
        ...
        if (_s == 1) {
            if (_hmu == 1) // Case 5, Case 6:
                return AisGEq(_RsTau, _RAsymTau) &&
                       AisLEq(_RsTau,
                              2 * vnl_math::pi); // should be
                                                 // AisLEq(_RsTau,_RintTau)
            else                                 // Case 7, Case 8:
                return AisGEq(_RsTau, vnl_math::pi) &&
                       AisLEq(_RsTau,
                              2 * vnl_math::pi); // should be
                                                 // AisGEq(_RsTau,_RintTau)
        } else {
            if (_emu == 1) // Case 9, Case 10
    ...
    if (_MU == 1) {    // same as Point-Arc
        ...
        if (_s == 1) {     // Inner Hyperbola, _nu==+1/-1 are the same
            if (_hmu == 1) // Case 5, Case 6:
                ...
                if (AisG(letau, vnl_math::pi)) // tau corresponding to min_eta
                                               // is out of range
                    letau = 0;
...
double dbsk2d_ishock_arcarc::computeMaxLTau() {
    if (_MU == 1) {    // Same as Point-Arc
        if (_s == 1) { // Case 1, Case 2:
            return vnl_math::min(_LAsymTau,
                                 LEtaToLTau(lBArc()->min_eta(), UNCONSTRAINED));
        } else {
            if (_nu == 1) { // Case 3:
                double letau = LEtaToLTau(lBArc()->min_eta(), UNCONSTRAINED);

                if (AisL(letau, vnl_math::pi)) // tau corresponding to min_eta
                                               // is out of range
                    letau = 2 * vnl_math::pi;
        ...
        if (_s == 1) {     // Inner Hyperbola, _nu==+1/-1 are the same
            if (_hmu == 1) // Case 5, Case 6:
                return vnl_math::min(
                    _LAsymTau, LEtaToLTau(lBArc()->min_eta(), UNCONSTRAINED));
            else // Case 7, Case 8:
                return _LsTau;
        } else {             // Inner Ellipse, _nu==+1/-1 are the same
            if (_emu == 1) { // Case 9, Case 10:
    ...
    if (_MU == 1) {    // Same as Point-Arc
        if (_s == 1) { // Case 1, Case 2:
            ...
            if (_nu == 1) { // Case 3:
                ...
                double retau = REtaToRTau(rBArc()->max_eta(), UNCONSTRAINED);
                if (AisG(retau, vnl_math::pi)) // tau corresponding to max_eta
                                               // is out of range
                    retau = 0;
    ...
    if (_MU == 1) {    // Same as Point-Arc
        ...
        if (_s == 1) {     // Inner Hyperbola, _nu==+1/-1 are the same
            if (_hmu == 1) // Case 5, Case 6:
                ...
                if (AisL(retau, vnl_math::pi)) // tau corresponding to max_eta
                                               // is out of range
                    retau = 2 * vnl_math::pi;
...
double dbsk2d_ishock_arcarc::RTau(double Ltau) {
    ...
    double rtau =
        vcl_atan2(d * vcl_sin(Ltau), -_H + d * vcl_cos(Ltau)) + vnl_math::pi;
...
double dbsk2d_ishock_arcarc::LTau(double Rtau) {
    ...
    double ltau = angle0To2Pi(vcl_atan2(d * vcl_sin(Rtau - vnl_math::pi),
                                        _H + d * vcl_cos(Rtau - vnl_math::pi)));
...
double dbsk2d_ishock_arcarc::EtaToTau(double eta, DIRECTION dir,
                                      bool constrained) {
    if (dir == LEFT)
...
double dbsk2d_ishock_arcarc::rp(double tau) { return 0; }

double dbsk2d_ishock_arcarc::rpp(double tau) { return 0; }

double dbsk2d_ishock_arcarc::g(double tau) { return 0; }
...
double dbsk2d_ishock_arcarc::k(double tau) { return 0; }
...
double dbsk2d_ishock_arcarc::v(double tau) {
    ...
    return vcl_fabs(vcl_sqrt(_a * _a + _c * _c - 2 * _a * _c * vcl_cos(tau)) /
                    (_c * vcl_sin(tau)));
}

double dbsk2d_ishock_arcarc::a(double tau) { return 0; }

double dbsk2d_ishock_arcarc::phi(double tau) { return 0; }
...
dbsk2d_ishock_edge::TAU_DIRECTION_TYPE dbsk2d_ishock_arcarc::tauDir() {
...
vgl_point_2d<double> dbsk2d_ishock_arcarc::getPtFromLTau(double tau) {
    ...
    pt = _origin +
         _rotateCCW(vgl_vector_2d<double>(d * vcl_cos(tau), d * vcl_sin(tau)),
                    _u);
...
vgl_point_2d<double> dbsk2d_ishock_arcarc::getLFootPt(double tau) {
    return _translatePoint(lBArc()->center(), angle0To2Pi(_u + tau),
                           lBArc()->R());
}
...
vgl_point_2d<double> dbsk2d_ishock_arcarc::getRFootPt(double tau) {
    ...
    return _translatePoint(rBArc()->center(),
                           angle0To2Pi(_u + vnl_math::pi + rtau), rBArc()->R());
}
...
void dbsk2d_ishock_arcarc::compute_extrinsic_locus() {
    ...
    // starting Point
    pt = _origin + _rotateCCW(vgl_vector_2d<double>(d(stau) * vcl_cos(stau),
                                                    d(stau) * vcl_sin(stau)),
                              _u);
    ...
    for (double tau = stau; tau <= etau; tau += DELTA_TAU) {
        pt = _origin + _rotateCCW(vgl_vector_2d<double>(d(tau) * vcl_cos(tau),
                                                        d(tau) * vcl_sin(tau)),
                                  _u);
    ...
    vgl_point_2d<double> e_pt =
        _origin + _rotateCCW(vgl_vector_2d<double>(d(etau) * vcl_cos(etau),
                                                   d(etau) * vcl_sin(etau)),
                             _u);
