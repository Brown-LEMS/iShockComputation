...
double dbsk2d_distance::point_to_bnd_edge(const vgl_point_2d<double>& query_pt,
                                          const dbsk2d_bnd_edge_sptr& edge,
                                          int direction, double s_start,
                                          double s_end) {
    ...
    if (bcurve->is_a() == "dbsk2d_ishock_bline") {
        ...
        return vgl_distance_to_linesegment<double>(
            (1 - smin) * start.x() + smin * end.x(),
            (1 - smin) * start.y() + smin * end.y(),
            (1 - smax) * start.x() + smax * end.x(),
            (1 - smax) * start.y() + smax * end.y(), query_pt.x(),
            query_pt.y());
    }
...
double
dbsk2d_distance::bnd_line_to_bnd_line(const dbsk2d_bnd_edge_sptr& bnd_line1,
    ...
    // the start and end of left_bcurve coincide with the start and end of the
    // bnd_edge
    return dbgl_distance::lineseg_lineseg(
...
double
dbsk2d_distance::vertex_to_bnd_line(const dbsk2d_bnd_vertex_sptr& vertex,
    ...
    return vgl_distance_to_linesegment<double>(p1.x(), p1.y(), p2.x(), p2.y(),
                                               p.x(), p.y());
}
