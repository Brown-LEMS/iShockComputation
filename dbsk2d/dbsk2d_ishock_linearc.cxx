...
dbsk2d_ishock_linearc::dbsk2d_ishock_linearc(int newid, double stime,
                                             dbsk2d_ishock_node* pse,
                                             dbsk2d_ishock_belm* lbe,
    ...
    if (LisEq(_H,
              0)) // Special degnerate case: Fortunately the line saves the day
        _u = bline->n();
    else
...
bool dbsk2d_ishock_linearc::isLSTauValid() {
    ...
    case 3:
        return AisGEq(_LsTau, 0) &&
               AisLEq(_LsTau,
                      vnl_math::pi); // should be AisGEq(_LsTau,_LintTau)
    case 5:
        return AisGEq(_LsTau, vnl_math::pi) &&
               AisLEq(_LsTau,
                      2 * vnl_math::pi); // should be AisLEq(_LsTau,_LintTau)
    case 7:
        return AisGEq(_LsTau, vnl_math::pi) &&
               AisLEq(_LsTau,
                      2 * vnl_math::pi); // should be AisGEq(_LsTau,_LintTau)
    case 9:
        return AisGEq(_LsTau, 0) &&
               AisLEq(_LsTau,
                      vnl_math::pi); // should be AisLEq(_LsTau,_LintTau)

    case 2:
        return LisGEq(_LsTau, 0) && LisLEq(_LsTau, _delta);
    case 4:
        return LisGEq(_LsTau, 0) &&
               LisLEq(_LsTau, _delta); // should be LisGEq(_LsTau,_LintTau)
    case 6:
        return LisGEq(_LsTau, 0) &&
               LisLEq(_LsTau, _l - _delta); // should be LisLEq(_LsTau,_LintTau)
    case 8:
        return LisGEq(_LsTau, 0) &&
               LisLEq(_LsTau, _delta); // should be LisGEq(_LsTau,_LintTau)
    case 10:
...
bool dbsk2d_ishock_linearc::isRSTauValid() {
    switch (_case) {
    case 1:
        return LisGEq(_RsTau, 0) && LisLEq(_RsTau, _l - _delta);
    case 3:
        return LisGEq(_RsTau, 0) &&
               LisLEq(_RsTau, _l - _delta); // should be LisGEq(_RsTau,_RintTau)
    case 5:
        return LisGEq(_RsTau, 0) &&
               LisLEq(_RsTau, _delta); // should be LisLEq(_RsTau,_RintTau)
    case 7:
        return LisGEq(_RsTau, 0) &&
               LisLEq(_RsTau, _l - _delta); // should be LisGEq(_RsTau,_RintTau)
    case 9:
    ...
    case 2:
        return AisGEq(_RsTau, vnl_math::pi) && AisLEq(_RsTau, 2 * vnl_math::pi);
    case 4:
        return AisGEq(_RsTau, vnl_math::pi) &&
               AisLEq(_RsTau,
                      2 * vnl_math::pi); // should be AisLEq(_RsTau,_RintTau)
    case 6:
        return AisGEq(_RsTau, 0) &&
               AisLEq(_RsTau,
                      vnl_math::pi); // should be AisGEq(_RsTau,_RintTau)
    case 8:
        return AisGEq(_RsTau, 0) &&
               AisLEq(_RsTau,
                      vnl_math::pi); // should be AisLEq(_RsTau,_RintTau)
    case 10:
        return AisGEq(_RsTau, vnl_math::pi) &&
               AisLEq(_RsTau,
                      2 * vnl_math::pi); // should be AisGEq(_RsTau,_RintTau)
    default:
        return false;
    }
}
...
double dbsk2d_ishock_linearc::computeMinLTau() {
    if (_nud == 1) { // Case 1, 2, 3, 4, 7, 8:
        ...
        if (_case == 5)
            return vnl_math::max(vnl_math::pi,
                                 LEtaToLTau(lBArc()->min_eta(), UNCONSTRAINED));

        else if (_case == 6 || _case == 10)
            return vnl_math::max(
                0.0, LEtaToLTau(lBLine()->min_eta(), UNCONSTRAINED));
        ...
        if (_nu == 1) {
            if (_s == 1) // Case 1, 3:
                ...
                if (AisL(letau, vnl_math::pi))
                    letau = 2 * vnl_math::pi; // tau corresponding to min_eta is
                                              // out of range
...
double dbsk2d_ishock_linearc::computeMinRTau() {
    if (_nu == 1) {
        if (_nud == 1) // Case 1, 3, 7:
            ...
            return vnl_math::max(
                0.0, REtaToRTau(rBLine()->max_eta(), UNCONSTRAINED));
    } else {
        if (_nud == 1) {
            ...
            if (_s == 1) { // Case 2, 4:
                if (AisL(retau, vnl_math::pi))
                    retau = vnl_math::pi; // tau corresponding to max_eta is out
                                          // of range
            } else {                      // Case 8:
    ...
    if (_nu == 1) {
        ...
        if (_nud == 1) // Case 2, 4, 8:
            ...
            if (_s == 1) { // Case 6:
                ...
                if (AisL(retau, vnl_math::pi))
                    retau = 2 * vnl_math::pi; // tau corresponding to max_eta is
                                              // out of range
            }
            return retau;
        }
...
double dbsk2d_ishock_linearc::RTau(double LTau) {
    ...
    if (_nu == 1) {
        ...
        if (LTau == 0) {
            ...
            double alpha;
            if (LTau > 0)
                alpha = vcl_atan(-2 * _c / LTau);
            else
                alpha = vcl_atan(-2 * _c / LTau) + vnl_math::pi;

            rtau = angle0To2Pi(
                vcl_acos(-LTau / vcl_sqrt(LTau * LTau + 4 * _c * _c)) + alpha);
            if (_s * _nud == 1)
...
double dbsk2d_ishock_linearc::LTau(double RTau) {
    ...
    if (_nu == -1) {
        ...
        if (RTau == 0) {
            ...
            double alpha;
            if (RTau > 0)
                alpha = vcl_atan(-2 * _c / RTau);
            else
                alpha = vcl_atan(-2 * _c / RTau) + vnl_math::pi;

            ltau = angle0To2Pi(
                vcl_acos(-RTau / vcl_sqrt(RTau * RTau + 4 * _c * _c)) + alpha);
...
double dbsk2d_ishock_linearc::EtaToTau(double eta, DIRECTION dir,
                                       bool constrained) {
    if (dir == LEFT)
...
double dbsk2d_ishock_linearc::LEtaToLTau(double eta, bool constrained) {
    ...
    if (_nu == 1) {
        ...
        // eta is the distance from the beginning of the line to the foot point
        // of the center of the arc
        ltau =
            _nud * (_delta -
                    eta); // for the inner shocks, parameterization is reversed
    }
...
double dbsk2d_ishock_linearc::REtaToRTau(double eta, bool constrained) {
    ...
    if (_nu == 1) {
        ...
        // delta is the signed distance from the beginning of the line to the
        // foot point of the center of the arc
        rtau = _nud *
               (eta -
                _delta); // for the inner shocks, parameterization is reversed
...
double dbsk2d_ishock_linearc::LTauToLEta(double tau, bool start) {
    ...
    if (_nu == 1) {
        ...
        // foot point of the center of the arc
        leta = _delta -
               _nud * tau; // for the inner shocks, parameterization is reversed
    }
...
double dbsk2d_ishock_linearc::g(double tau) {
    return _H * vcl_sqrt(2 / ((1 + vcl_cos(tau)) * (1 + vcl_cos(tau)) *
                              (1 + vcl_cos(tau))));
}

double dbsk2d_ishock_linearc::k(double tau) { return _H / vcl_sqrt(8.0); }
...
double dbsk2d_ishock_linearc::v(double tau) // FIX ME
{
    ...
    case 1:
        if (tau == 0)
            return 100000; // code for infinity
    case 2:
        if (tau == 2 * vnl_math::pi)
            return 100000;
    case 3:
        break; // ignore infinity should bever happen
    case 4:
        break; // ignore infinity should bever happen
    case 5:
        if (tau == vnl_math::pi)
    ...
    case 8:
        if (tau == 2 * vnl_math::pi)
            return 100000;
    case 9:
        break; // ignore infinity should bever happen, same as 3
    case 10:
        break; // ignore infinity should bever happen, same as 4
    case 11:
        if (tau == 0)
            return 100000; // same as 7
    case 12:
        if (tau == 2 * vnl_math::pi)
            return 100000; // same as 8
    }
    if (_s * _nud == 1)
        return vcl_fabs(vcl_sqrt(2 + 2 * vcl_cos(tau)) / vcl_sin(tau));
    else
        return vcl_fabs(
            vcl_sqrt(2 + 2 * vcl_cos(tau + vnl_math::pi)) /
            vcl_sin(
                tau +
                vnl_math::
                    pi)); // vcl_fabs(vcl_sqrt(2-2*vcl_cos(tau))/vcl_sin(tau));
}

double dbsk2d_ishock_linearc::a(double tau) { return 0; }

double dbsk2d_ishock_linearc::phi(double tau) { return 0; }
...
dbsk2d_ishock_edge::TAU_DIRECTION_TYPE dbsk2d_ishock_linearc::tauDir() {
    if (_nu == 1) {
        if (_nud == 1)
            return TAU_INCREASING;
        else
            return TAU_DECREASING;
    } else {
        if (_nud == 1)
            return TAU_DECREASING;
        else
...
vgl_point_2d<double> dbsk2d_ishock_linearc::getPtFromPointTau(double ptau) {
    ...
    vgl_point_2d<double> pt =
        _origin +
        _rotateCCW(vgl_vector_2d<double>(d * vcl_cos(ptau), d * vcl_sin(ptau)),
                   _u);
...
vgl_point_2d<double> dbsk2d_ishock_linearc::getMidPt(void) {
    if (_endTime > MAX_RADIUS)
        return getPtFromPointTau((sTau() + getPointTauFromTime(MAX_RADIUS)) /
                                 2);
    else
...
vgl_point_2d<double> dbsk2d_ishock_linearc::getLFootPt(double ptau) {
    if (_nu == 1)
        return _translatePoint(lBArc()->center(), angle0To2Pi(_u + ptau),
                               lBArc()->R());
    else {
        // return _getFootPt (getPtFromTau(ptau), lBLine()->start(),
        // lBLine()->end());
...
vgl_point_2d<double> dbsk2d_ishock_linearc::getRFootPt(double ptau) {
    if (_nu == -1)
        return _translatePoint(rBArc()->center(), angle0To2Pi(_u + ptau),
                               rBArc()->R());
    else {
        // return _getFootPt (getPtFromTau(ptau), rBLine()->start(),
        // rBLine()->end());
...
void dbsk2d_ishock_linearc::compute_extrinsic_locus() {
    ...
    d = H / (1 + vcl_cos(stau));
    pt = _origin +
         _rotateCCW(vgl_vector_2d<double>(d * vcl_cos(stau), d * vcl_sin(stau)),
                    u);
    ...
    for (double tau = stau; tau <= etau; tau += DELTA_TAU) {
        ...
        pt = _origin +
             _rotateCCW(
                 vgl_vector_2d<double>(d * vcl_cos(tau), d * vcl_sin(tau)), u);
    ...
    d = H / (1 + vcl_cos(etau));
    vgl_point_2d<double> e_pt =
        _origin +
        _rotateCCW(vgl_vector_2d<double>(d * vcl_cos(etau), d * vcl_sin(etau)),
                   u);
...
void dbsk2d_ishock_linearc::getInfo(vcl_ostream& ostrm) {
    ...
    ostrm << "lB: " << _lBElement->id();
    vcl_sprintf(s, " (%f, %f)\n", LsEta(), LeEta());
    ostrm << s;
    ostrm << "rB: " << _rBElement->id();
    vcl_sprintf(s, " (%f, %f)\n", RsEta(), ReEta());
    ostrm << s;
    ostrm << "[ lS: " << (_lShock ? _lShock->id() : -1)
          << ", rS: " << (_rShock ? _rShock->id() : -1) << "]\n";
    ostrm << "[ lN: " << (_lNeighbor ? _lNeighbor->id() : -1)
          << ", rN: " << (_rNeighbor ? _rNeighbor->id() : -1);
    ostrm << ", pN: " << _pSNode->id()
          << ", cN: " << (_cSNode ? _cSNode->id() : -1) << " ]\n"
          << vcl_endl;
    ...
    vcl_sprintf(s, "LEta: %f - %f (Eta range: %f - %f)\n", LsEta(), LeEta(),
                _lBElement->min_eta(), _lBElement->max_eta());
    ostrm << s;
    vcl_sprintf(s, "REta: %f - %f (Eta range: %f - %f)\n\n", RsEta(), ReEta(),
                _rBElement->min_eta(), _rBElement->max_eta());
    ostrm << s;
    ...
    if (_cSNode)
        ...
        if (this->cell_bnd()) {
            if (this->cell_bnd()->is_vert())
                ostrm << "at vert. bnd; y=" << this->bnd_intersect_pos()
                      << vcl_endl;
            else
                ostrm << "at horiz. bnd; x=" << this->bnd_intersect_pos()
                      << vcl_endl;
        } else
