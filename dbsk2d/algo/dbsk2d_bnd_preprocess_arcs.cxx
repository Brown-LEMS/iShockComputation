...
// further processing
void dbsk2d_bnd_preprocess::intersect_bnd_arcs(
    vcl_list<dbsk2d_bnd_edge_sptr>* tainted_arcs,
    ...
    for (bnd_edge_list::iterator eit1 = arcset1->begin();
        ...
        // no need to start from begining when intersecting arcset1 against
        // itself
        else {
            eit2 = eit1;
            ++eit2;
        ...
        while (eit2 != arcset2b->end()) {
            ...
            // because their intersections have been recorded, hence...`shared
            // vertices'
            ...
            dbsk2d_ishock_barc* barc2 =
                static_cast<dbsk2d_ishock_barc*>(e2->left_bcurve());
            ...
            // ignore intersection at endpoints. It is handled in a different
            // fnct.
            for (unsigned int i = 0; i < arc1_ratios.size(); ++i) {
                ...
                vgl_point_2d<double> new_pt = centre<double>(
                    //: ozge changed
                    // arc1.point_at_length(arc1_ratios[i]),
                    // arc2.point_at_length(arc2_ratios[i]));

                    //: Nhon changed back
                    arc1.point_at(arc1_ratios[i]),
                    arc2.point_at(arc2_ratios[i]));
...
void dbsk2d_bnd_preprocess::remove_duplicate_arcs(
    ...
    for (bnd_edge_list::iterator eit1 = bnd_arcs.begin();
        ...
        while (eit2 != bnd_arcs.end()) {
            ...
            dbgl_circ_arc arc2(barc2->start(), barc2->end(),
                               barc2->curvature());

            // computing max distance difference depending on signs of
            // curvatures of two arcs, given they share same end points
            double max_distance = -1;
            if ((arc1.k() < 0 && arc2.k() > 0) ||
                (arc1.k() > 0 && arc2.k() < 0)) {
            ...
            // distance falls below threshold, one (e2) has to go. Replace e2 by
            // e1.
            e1->merge_with(*e2);
