...
bool dbsk2d_bnd_preprocess::preprocess(dbsk2d_boundary_sptr boundary,
                                       bool talkative) {
            ...
            for (int j = 0; j < boundary->num_cols(); ++j) {
                ...
                if (talkative) {
                    vcl_cout << "Cell row=" << i << " column= " << j
                             << "  number of edges= " << cell->num_bnd_edges()
                             << vcl_endl;
                ...
                if (talkative) {
                    vcl_cout << "done\n";
                    vcl_cout << "Number of edges after preprocessing="
                             << cell->edges().size() << vcl_endl;
                }
            }
    }
...
bool dbsk2d_bnd_preprocess::need_preprocessing(dbsk2d_boundary_sptr boundary) {
    dbsk2d_assert(boundary);
    this->set_boundary(boundary);
...
// Return false if preprocessing fails
bool dbsk2d_bnd_preprocess::preprocess(vcl_list<dbsk2d_bnd_edge_sptr>& edges) {
    ...
    dbsk2d_bnd_utils::extract_vertex_list(bnd_arcs, arc_vertices);
    this->dissolve_vertices_into_curves(affected_lines, bnd_lines,
                                        arc_vertices);
    arc_vertices.clear();
    this->remove_short_curves(affected_lines);
    this->remove_duplicate_lines(affected_lines);
...
bool dbsk2d_bnd_preprocess::need_preprocessing(
...
bool dbsk2d_bnd_preprocess::points_need_preprocessing(
...
bool dbsk2d_bnd_preprocess::lines_need_preprocessing(
    ...
    for (bnd_edge_list::const_iterator eit = bnd_lines.begin();
        ...
        for (bnd_vertex_list::iterator vit = line_vertices.begin();
            ...
            if (e->is_endpoint(v.ptr()))
                continue;
            vgl_point_2d<double> pt = v->point();
            double d = vgl_distance_to_linesegment(p1.x(), p1.y(), p2.x(),
                                                   p2.y(), pt.x(), pt.y());
...
//----------------------------------------------------------------------
//: Return true if this set of line edges need preprocessor
bool dbsk2d_bnd_preprocess::point_lines_need_preprocessing(
    const vcl_list<dbsk2d_bnd_edge_sptr>& bnd_lines,
    const vcl_list<dbsk2d_bnd_edge_sptr>& bnd_pts) {
...
//
// void dbsk2d_boundary::
// PreProcessBoundaryForEdgeInput(double position_accuracy, double
// angle_accuracy,
//     double operator_width, double operator_length)
//{
//   //this function is trying to weed out redundant edges that subpixel edge
//   detectors like
//   //L/L often output. These edges unnecessarily increase the complexity of
//   the grouping
//   //process and often times crashes the DT code
//
//   //0. Remove edge of the image responses.
//   //1. if two edges are really close, then replace them with an edge that is
//   in between them
...
//   //          position accuracy
//   //        ii. if they are seperated in the tangential direction by less
//   than
//   //          the position accuracy
//   //3. If an edge is close to two edges that are not close to one another
//   then remove it.
//   //4. Definition of close:
//   //      a. If the tangents agree within the angle inaccuracy and the edge
//   in the
//   //        middle is less than half the operator length away from both the
//   edges.
//
...
//             dbsk2d_ishock_bpoint* bp1 =
//             (dbsk2d_ishock_bpoint*)BElmList[(*m)]; dbsk2d_ishock_bpoint* bp2
//             = (dbsk2d_ishock_bpoint*)BElmList[(*n)];
//
