...
// further processing
void dbsk2d_bnd_preprocess::intersect_lines_against_arcs(
    vcl_list<dbsk2d_bnd_edge_sptr>* tainted_edges,
        ...
        for (bnd_edge_list::iterator eit2 = arcset->begin();
            ...
            for (unsigned int i = 0; i < arc_ratios.size(); ++i) {
                ...
                vgl_point_2d<double> new_pt =
                    centre<double>(midpoint<double>(p11, p12, line_ratios[i]),

                                   //: ozge changed point_at to point_at_length
                                   // arc2.point_at_length(arc_ratios[i]));

                                   //: Nhon changed back to point_at(...)
                                   //: because point_at(t) takes in t \in [0,1]
...
// -----------------------------------------------------------------------
//: Remove arcs that share both vertices with another line and the maximum
//: distance
// between the arc and the line is less than distance threshold
void dbsk2d_bnd_preprocess::remove_arcs_duplicating_lines(
    ...
    while (eit1 != bnd_arcs.end()) {
        ...
        for (bnd_edge_list::const_iterator eit2 = bnd_lines.begin();
            ...
            dbgl_circ_arc arc1(barc1->start(), barc1->end(),
                               barc1->curvature());
