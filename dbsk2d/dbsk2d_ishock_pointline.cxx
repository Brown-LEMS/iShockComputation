...
dbsk2d_ishock_pointline::dbsk2d_ishock_pointline(
    int newid, double stime, dbsk2d_ishock_node* pse, dbsk2d_ishock_belm* lbe,
    dbsk2d_ishock_belm* rbe, double lsEta, double rsEta, bool constrained)
    ...
    if (_nu > 0) {
        ...
        _l = bline->l();
        _H = _distPointLine(bpoint->pt(), bline->start(), bline->end());
        dbsk2d_assert(_H != 0);
        _origin = bpoint->pt();

        // CW angle from vref of the corresponding point to the corresponding
        // axis vector
        _ldelta = CCW(_u, bpoint->vref());
        // signed distance from bline->start to the foot-point
        _rdelta = _deltaPointLine(bpoint->pt(), bline->start(), bline->end(),
                                  bline->l());
    } else {
        bpoint = rBPoint();
        bline = lBLine();
        ...
        // signed distance from bline->start to the foot-point
        _ldelta = _deltaPointLine(bpoint->pt(), bline->start(), bline->end(),
                                  bline->l());
        // CW angle from vref of the corresponding point to the corresponding
        // axis vector
...
void dbsk2d_ishock_pointline::correct_intrinsic_parameters_at_init() {
    ...
    if (_nu > 0) {
        ...
        if (LisG(alt_rstau, _RsTau)) // update _RsTau to the alternate value
            _RsTau = alt_rstau;
        else if (LisL(alt_rstau, _RsTau)) // update _LsTau to the alternate
                                          // value
            _LsTau = LTau(_RsTau);
    } else {
        ...
        if (LisG(alt_lstau, _LsTau)) // update _LsTau to the alternate value
            _LsTau = alt_lstau;
        else if (LisL(alt_lstau, _LsTau)) // update _RsTau to the alternate
                                          // value
            _RsTau = RTau(_LsTau);
    }
}
...
double dbsk2d_ishock_pointline::RTau(double LTau) {
    if (_nu == 1) {
        ...
        // return 2*vnl_math::pi - angle0To2Pi(vcl_acos((_H*_H -
        // LTau*LTau)/(_H*_H + LTau*LTau))); //too sensitive
        if (AisEq(LTau, 0))
            return 2 * vnl_math::pi;

        return 2 * vnl_math::pi -
               angle0To2Pi(vcl_acos(-LTau / vcl_sqrt(LTau * LTau + _H * _H)) +
                           vcl_atan(-_H / LTau));
    }
}
...
double dbsk2d_ishock_pointline::LTau(double RTau) {
    if (_nu == -1) {
        ...
        // return angle0To2Pi(vcl_acos((_H*_H - RTau*RTau)/(_H*_H +
        // RTau*RTau))); //too sensitive
        if (AisEq(RTau, 0))
            return 0.0;

        return angle0To2Pi(vcl_acos(-RTau / vcl_sqrt(RTau * RTau + _H * _H)) +
                           vcl_atan(-_H / RTau));
    }
}
...
double dbsk2d_ishock_pointline::EtaToTau(double eta, DIRECTION dir,
                                         bool constrained) {
...
double dbsk2d_ishock_pointline::REtaToRTau(double eta, bool constrained) {
    ...
    if (constrained) {
        if (_nu == 1) {
            ...
            if (rtau < vnl_math::pi)
                rtau =
                    2 * vnl_math::pi; // move it to the start of the valid range
        }
...
double dbsk2d_ishock_pointline::LTauToLEta(double tau, bool start) {
    ...
    if (_nu == 1) {
        ...
        double leta =
            this->lBPoint()->vec_to_eta(_u + tau); // range [min_eta..max_eta]
...
double dbsk2d_ishock_pointline::RTauToREta(double tau, bool start) {
    ...
    if (_nu == 1)
        ...
        double reta =
            this->rBPoint()->vec_to_eta(_u + tau); // range [min_eta..max_eta]
...
double dbsk2d_ishock_pointline::k(double tau) { return _H / vcl_sqrt(8.0); }
...
double dbsk2d_ishock_pointline::a(double tau) { return 0; }
...
vgl_point_2d<double> dbsk2d_ishock_pointline::getLFootPt(double ptau) {
    if (_nu == 1)
        ...
        return _getFootPt(getPtFromTau(ptau),
                          ((dbsk2d_ishock_bline*)_lBElement)->start(),
                          ((dbsk2d_ishock_bline*)_lBElement)->end());
}
...
vgl_point_2d<double> dbsk2d_ishock_pointline::getRFootPt(double ptau) {
    if (_nu == -1)
        ...
        return _getFootPt(getPtFromTau(ptau),
                          ((dbsk2d_ishock_bline*)_rBElement)->start(),
                          ((dbsk2d_ishock_bline*)_rBElement)->end());
}
...
void dbsk2d_ishock_pointline::compute_extrinsic_locus() {
    ...
    // 2)We have to first avoid the Extreme Condition that the parabola goes to
    // infinity
    ...
    while ((_nu == 1 && tau < etau) || (_nu == -1 && tau > etau)) {
        ...
        pt = _origin +
             _rotateCCW(
                 vgl_vector_2d<double>(d * vcl_cos(tau), d * vcl_sin(tau)), _u);
