...
dbsk2d_compute_ishock_process()
{
    ...
    if (
            ...
            */
		    ...
		    !parameters()->add( "If add bounding box based on image size: ", "-img_bbox", (bool)true))
...
execute()
{
	...
	bool img_bbox;
	parameters()->get_value("-img_bbox", img_bbox);
	//add bounding box based on image size
	if(img_bbox)
	{
...
//	vcl_ofstream outfp("belm_list.txt", vcl_ios::out);
//	boundary->print_belm_list(outfp);
//	outfp.close();
//	int a;
//	vcl_cin >> a;
