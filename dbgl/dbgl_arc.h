...
//  but depends on the direction of arc (ccw = 1 if counterclockwise, -1
//  otherwise) alpha(s) = alpha0 + [ [ 2PI + ccw * (alpha1-alpha0) ] % 2PI ] *
//  s)
class dbgl_arc : public dbgl_arc_base {
  public:
    static constexpr double
        near_zero_value VCL_STATIC_CONST_INIT_FLOAT_DECL(1.0e-8);
    static constexpr double minimum_curvature
        VCL_STATIC_CONST_INIT_FLOAT_DECL(0.00260416); //(1/384.0)

    //: default constructor
    dbgl_arc()
        : center_(0, 0), curv_(1.0), ccw_(1), alpha0_(0.0), alpha1_(PI) {}
    ...
    dbgl_arc(const vgl_point_2d<double>& start, const vgl_point_2d<double>& end,
             const vgl_point_2d<double>& other);

    //:
    virtual ~dbgl_arc();

    static const vcl_type_info& type_id() { return typeid(dbgl_arc); }
    ...
    virtual bool operator==(const dbgl_arc& c) const;
    inline bool operator!=(const dbgl_arc& other) const {
        return !operator==(other);
    }

    // bool operator==(dbgl_param_curve const& obj) const; // virtual of
    // dbgl_param_curve
    ...
    //: Get sample point at value s of a parameter along the curve, s within
    //: [0,1]
  ...
  protected: // some routines that maybe useful for friends and derived classes
    ...
    static unsigned compute_inters_ll(const vgl_point_2d<double>& p11,
                                      const vgl_point_2d<double>& p12,
                                      const vgl_point_2d<double>& p21,
                                      const vgl_point_2d<double>& p22,
                                      vgl_point_2d<double>* inter);

  private:
    bool find_center(const vgl_point_2d<double>& p11,
                     const vgl_point_2d<double>& p12,
                     const vgl_point_2d<double>& p21,
                     const vgl_point_2d<double>& p22);
    //: center. For 0-curvature arcs, this is the middle point of the line
    //: segment
    ...
    int ccw_;
    //: starting angle in [0,2PI]. But for 0-curvature arcs, this is
    //: x-coordinate of the start point
    double alpha0_;
    //: ending angle in [0,2PI]. But for 0-curvature arcs, this is y-coordinate
    //: of the start point
    double alpha1_;
};
...
inline bool dbgl_arc::is_almost_equal(double val1, double val2,
                                      double tolerance) {
...
inline double dbgl_arc::length() const {
    if (curv_ != 0.0) {
        ...
        return 2 *
               vnl_math::hypot(center_.x() - alpha0_, center_.y() - alpha1_);
}
