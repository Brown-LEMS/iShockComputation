...
const int dbgl_eulerspiral_max_gradient_descent_iter =
    50000; // maxinum number of iterations for gradient descent
...
class dbgl_eulerspiral_optimization_function
  ...
  private:
    //: disabled contructor, can only construct this function inside an
    //: eulerspiral class
...
dbgl_eulerspiral::dbgl_eulerspiral(vgl_point_2d<double> start,
                                   double start_angle, double k0, double gamma,
                                   double len)
...
dbgl_eulerspiral::dbgl_eulerspiral(vgl_point_2d<double> start,
                                   double start_angle, vgl_point_2d<double> end,
                                   double end_angle) {
...
//: Copy Constructor
dbgl_eulerspiral::dbgl_eulerspiral(const dbgl_eulerspiral& es)
    : start_(es.start_), k0_(es.k0_), gamma_(es.gamma_), len_(es.len_) {
...
void dbgl_eulerspiral::set_start_angle(double start_angle) {
...
void dbgl_eulerspiral::set_end_angle(double end_angle) {
...
vgl_point_2d<double> dbgl_eulerspiral::point_at_length(double s) const {
    return this->compute_end_pt(this->k0(), this->gamma(), s);
}

vgl_point_2d<double> dbgl_eulerspiral::point_at(double s) const {
    return point_at_length(s * length());
}

//: Return tangent of a point on the eulerspiral with s arclength away from
//: starting point
vgl_vector_2d<double> dbgl_eulerspiral::tangent_at_length(double s) const {
    double angle;
    angle = this->start_angle() + s * (this->k0() + 0.5 * this->gamma() * s);
    return vgl_vector_2d<double>(vcl_cos(angle), vcl_sin(angle));
}

vgl_vector_2d<double> dbgl_eulerspiral::tangent_at(double s) const {
...
double dbgl_eulerspiral::tangent_angle_at_length(double s) const {
    double angle =
        this->start_angle() + s * (this->k0() + 0.5 * this->gamma() * s);
...
//: Return curvature of a point on the eulerspiral with s arclength away from
//: starting point
double dbgl_eulerspiral::curvature_at_length(double s) const {
    return this->k0() + s * this->gamma();
}

double dbgl_eulerspiral::curvature_at(double s) const {
    return curvature_at_length(s * length());
}

//: compute extrinsic points of eulerspiral
void dbgl_eulerspiral::compute_spiral(vcl_vector<vgl_point_2d<double>>& spiral,
                                      double ds, int npts) {
...
void dbgl_eulerspiral::init(const vgl_point_2d<double>& start,
                            double start_angle, double k0, double gamma,
                            double len) {
...
void dbgl_eulerspiral::set_intrinsic_params(double k0, double gamma,
                                            double len) {
    this->set_k0(k0);
    this->set_gamma(gamma);
    this->set_len(len);
...
void dbgl_eulerspiral::set_extrinsic_params(const vgl_point_2d<double>& start,
                                            double start_angle) {
    this->set_start(start);
    this->set_start_angle(start_angle);
}
...
void dbgl_eulerspiral::set_start_params(vgl_point_2d<double> start,
                                        double start_angle) {
    this->set_start(start);
    this->set_start_angle(start_angle);
}
...
void dbgl_eulerspiral::set_end_params(vgl_point_2d<double> end,
                                      double end_angle) {
    this->set_end(end);
...
void dbgl_eulerspiral::set_params(vgl_point_2d<double> start,
                                  double start_angle, vgl_point_2d<double> end,
                                  double end_angle) {
    this->set_start_params(start, start_angle);
    this->set_end_params(end, end_angle);
...
//: Compute intrinsic parameters of eulerspiral using starting and ending
//: point-tangents
bool dbgl_eulerspiral::compute_es_params(bool use_lookup_table,
                                         bool use_levenberg_marquardt) {
    if (use_levenberg_marquardt)
        return this->compute_es_params_use_levenberg_marquardt(
            use_lookup_table);
    else
        return this->compute_es_params_use_simple_gradient_descent(
            use_lookup_table);
}

//: Compute intrinsic parameters of eulerspiral given two point-tangents
bool dbgl_eulerspiral::compute_es_params(vgl_point_2d<double> start,
                                         double start_angle,
...
bool dbgl_eulerspiral::operator==(const dbgl_eulerspiral& that) const {
    return (this->start() == that.start() &&
            this->start_angle() == that.start_angle() &&
            this->k0() == that.k0() && this->gamma() == that.gamma() &&
            this->length() == that.length());
};

//: assignment operator
dbgl_eulerspiral& dbgl_eulerspiral::operator=(dbgl_eulerspiral const& that) {
    this->init(that.start(), that.start_angle(), that.k0(), that.gamma(),
               that.length());
    return (*this);
}
...
void dbgl_eulerspiral::print(vcl_ostream& os) {
    os << vcl_endl << "Start parameters" << vcl_endl;
    os << "Start point = ( " << this->start().x() << " , " << this->start().y()
       << " )" << vcl_endl;
    os << "Start angle = " << this->start_angle() << vcl_endl;
    os << "End parameters" << vcl_endl;
    os << "End point = ( " << this->end().x() << " , " << this->end().y()
       << " )" << vcl_endl;
    ...
    os << "Number of iterations for last optimization = "
       << this->num_iterations() << vcl_endl;
...
//: Return error: euclidean distance between end-point of the Euler spiral with
//: the given
// intrinsic params and the desired end-point
inline double dbgl_eulerspiral::compute_error(double k0, double len) {
    // assumes normalized parameters
    // compute the endpoint of the Euler spiral with the given intrinsic
    // parameters
    double gamma = 2 * (this->turning_angle_ - k0 * len) / (len * len);
    vgl_point_2d<double> cur_end_pt =
        this->compute_end_pt(k0, gamma, len, true);

    // the error is the distance between the current end point and the desired
    // end point
...
vgl_point_2d<double> dbgl_eulerspiral::compute_end_pt(double k0, double gamma,
                                                      double len,
    ...
    if (normalized) {
        start_pt.set(0, 0);
        // convert this->psi - this->start_angle to the range [0, 2pi) and
        // assign to theta
    ...
    if (vcl_fabs(gamma) < dbgl_eulerspiral_e_gamma) {
        if (vcl_fabs(k0) < dbgl_eulerspiral_e_k) {
            ...
            end_pt =
                start_pt +
                vgl_vector_2d<double>(vcl_cos(theta), vcl_sin(theta)) * len;
        } else {
            // circle
            double const_term = 1.0 / k0;
            end_pt.set(start_pt.x() + const_term * (vcl_sin(k0 * len + theta) -
                                                    vcl_sin(theta)),
                       start_pt.y() + const_term * (-vcl_cos(k0 * len + theta) +
                                                    vcl_cos(theta)));
        }
        return end_pt;
    }
    ...
    dbnl_fresnel_integral((k0 + gamma * len) / const1, &fresnel1_cos,
                          &fresnel1_sin);
...
//: compute Euler spiral core intrinsic parameters from starting and ending
//: point-tangent using
// simple gradient descent algorithm
bool dbgl_eulerspiral::compute_es_params_use_simple_gradient_descent(
    ...
    // check if the lookup table is available, if yes then use it, otherwise
    // initialize using biarc
    if (use_lookup_table) {
        if (dbgl_eulerspiral_lookup_table::instance()->has_table()) {
            ...
            // the correct turning angle should be close to the estimated
            // turning angle
            turning_angle_est =
                k0_est * len_est + 0.5 * gamma_est * len_est * len_est;
            int num_offset_cycle;
            num_offset_cycle = vnl_math::rnd(
                (this->end_angle() - this->start_angle() - turning_angle_est) /
                (vnl_math::pi * 2));
            this->turning_angle_ = (this->end_angle() - this->start_angle()) -
                                   num_offset_cycle * vnl_math::pi * 2;
            k0_init_est = k0_est;
    ...
    // if not using look-up table or lookup table is not available then
    // initialize using biarc
    if (!k0_len_initialized) {
        ...
        // check if a biarc can be computed. If yes then initialize using the
        // biarc and move on if no, then there is no euler spiral either
        if (!biarc_estimate.compute_biarc_params(
                this->start(), this->start_angle(), this->end(),
                this->end_angle())) {
            this->set_intrinsic_params(0, 0, vnl_huge_val((double)1.0));
            return false;
        }
        ...
        // From here on, normlize the parameters and use these to perform the
        // optimization
    ...
    this->set_k0(k0 / d);
    this->set_len(len * d);
    this->set_gamma(2 * (this->turning_angle_ - k0 * len) / (len * len) /
                    (d * d));
    this->error_ = error;
    return true;
}

bool dbgl_eulerspiral::compute_es_params_use_levenberg_marquardt(
    bool use_lookup_table) {
    ...
    dbgl_eulerspiral_optimization_function es_optim_funct(
        2, 2, vnl_least_squares_function::no_gradient);
    es_optim_funct.set_es(this);

    // initialize root vector
    vnl_vector<double> root_vector = k0_len_estimate;

    // solve for root using nonlinear minimization, vnl_levenberg_marquardt
    // algorithm
...
const vcl_string dbgl_eulerspiral_lookup_table::file_path =
    vcl_string(LEMSVXL_LIB_DIR);
#else
const vcl_string dbgl_eulerspiral_lookup_table::file_path = vcl_string();
#endif

const vcl_string dbgl_eulerspiral_lookup_table::file_name =
    vcl_string("dbgl_eulerspiral_lookup_table.bvl");
...
dbgl_eulerspiral_lookup_table::dbgl_eulerspiral_lookup_table() {
    vcl_string full_path = dbgl_eulerspiral_lookup_table::file_path +
                           vcl_string("/") +
...
//: delta theta values for the table (tells you about the accuracy of the
//: lookup)
double dbgl_eulerspiral_lookup_table::dt() { return dt_; }

//: look up values of k0, gamma, and len of a normalized euler spiral given
// starting and ending angle. Perform bilear interpolation when in between
// sampled points
void dbgl_eulerspiral_lookup_table::look_up(double start_angle,
                                            double end_angle, double* k0,
                                            double* gamma, double* len,
                                            double* k0_max_error,
                                            double* gamma_max_error,
                                            double* len_max_error) {
    ...
    start_angle =
        (start_angle < 0) ? start_angle + 2 * vnl_math::pi : start_angle;
    ...
    *k0 = (1 - a) * (1 - b) * x11 + a * (1 - b) * x21 + (1 - a) * b * x12 +
          a * b * x22;
    *k0_max_error = vnl_math::max(vnl_math::max((double)x11, (double)x21),
                                  vnl_math::max((double)x12, (double)x22)) -
                    vnl_math::min(vnl_math::min((double)x11, (double)x21),
    ...
    *gamma = (1 - a) * (1 - b) * x11 + a * (1 - b) * x21 + (1 - a) * b * x12 +
             a * b * x22;
    *gamma_max_error = vnl_math::max(vnl_math::max((double)x11, (double)x21),
                                     vnl_math::max((double)x12, (double)x22)) -
                       vnl_math::min(vnl_math::min((double)x11, (double)x21),
                                     vnl_math::min((double)x12, (double)x22));
    ...
    *len = (1 - a) * (1 - b) * x11 + a * (1 - b) * x21 + (1 - a) * b * x12 +
           a * b * x22;
    *len_max_error = vnl_math::max(vnl_math::max((double)x11, (double)x21),
                                   vnl_math::max((double)x12, (double)x22)) -
                     vnl_math::min(vnl_math::min((double)x11, (double)x21),
                                   vnl_math::min((double)x12, (double)x22));
...
dbgl_eulerspiral_optimization_function::dbgl_eulerspiral_optimization_function(
    unsigned int number_of_unknowns, unsigned int number_of_residuals,
    UseGradient g)
...
void dbgl_eulerspiral_optimization_function::f(vnl_vector<double> const& x,
    ...
    vgl_point_2d<double> cur_end_pt =
        this->es()->compute_end_pt(k0, gamma, len, true);
    // return vector
