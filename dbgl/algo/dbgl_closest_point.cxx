...
double dbgl_closest_point::lineseg_lineseg(const vgl_point_2d<double>& line1_p1,
    ...
    if (vgl_lineseg_test_lineseg<double>(
        ...
        if (vcl_abs(pd) < 1e-12) {
            // check if the first point of line1 is between two end points of
            // line2 if so choose as intersection point
            ...
            else if (dot_product(line1_p2 - line2_p1, line1_p2 - line2_p2) <=
                     0) {
                px = line1_p2.x();
                py = line1_p2.y();
                vgl_point_2d<double> intersect_pt(px, py);
            ...
            // if both tests above fail, that means that both end points of
            // line2 are between two end points of line1 (line1 contains line2).
            // Choose starting point of line2 as intersection point
            else {
                px = line2_p1.x();
                py = line2_p1.y();
                vgl_point_2d<double> intersect_pt(px, py);
        ...
        // case 1b: two lines actually intersect (normal case)
            ...
            // parameters in range [0, 1] of intersecting points along line1 and
            // line2 line 1
            ratio1 = ratio<double>(line1_p1, line1_p2, intersect_pt);
    ...
    // distances from one line endpoint to the other line segment.
        ...
        // put all cases into an array so that we don't have to trace them
        // later.
        double xy[][6] = {{line2_p1.x(), line2_p1.y(), line2_p2.x(),
        ...
        double d[4];
        for (int i = 0; i < 4; ++i) {
            d[i] = vgl_distance2_to_linesegment<double>(
        ...
        double xp, yp;
        vgl_closest_point_to_linesegment<double>(
            xp, yp, xy[order][0], xy[order][1], xy[order][2], xy[order][3],
            xy[order][4], xy[order][5]);

        // length ratio from (xp,yp) to starting point of its segment
        double ratio_pt =
            ratio<double>(vgl_point_2d<double>(xy[order][0], xy[order][1]),
...
// on the arc
double dbgl_closest_point::point_to_circular_arc(
    const vgl_point_2d<double>& query_pt, const vgl_point_2d<double>& p1,
    const vgl_point_2d<double>& p2, double k, double& ret_ratio) {
    // degenerate arc: a line segment
    if (vnl_math::abs(k) < dbgl_closest_point::min_curvature) {
        double xm, ym;
        vgl_closest_point_to_linesegment<double>(
            xm, ym, p1.x(), p1.y(), p2.x(), p2.y(), query_pt.x(), query_pt.y());
    ...
    if (ret_ratio >= 0 && ret_ratio <= 1) {
        ...
        dist_to_point[0] =
            vnl_math::hypot(p1.x() - query_pt.x(), p1.y() - query_pt.y());
        dist_to_point[1] =
            vnl_math::hypot(p2.x() - query_pt.x(), p2.y() - query_pt.y());
...
double dbgl_closest_point::point_to_circle(const vgl_point_2d<double>& query_pt,
                                           const vgl_point_2d<double>& p0,
                                           const vgl_vector_2d<double>& t0,
                                           double k, double& ret_arc_length) {
...
double dbgl_closest_point::lineseg_to_circular_arc(
    const vgl_point_2d<double>& line_p1, const vgl_point_2d<double>& line_p2,
    const vgl_point_2d<double>& arc_p1, const vgl_point_2d<double> arc_p2,
    double arc_k, vcl_vector<double>& line_ratios,
    ...
    if (roots.size() > 0) {
        ...
        for (unsigned int i = 0; i < roots.size(); ++i) {
            ...
            if (!(0 <= ratio_si && ratio_si <= 1))
                continue;

            // range check passed. This is a valid intersection point. Record
            // it.
    ...
    // need to find closest point between the CIRCLE and the LINE, the perfor
    // range check
    else {
        // parameter (t) of closest point on line is the average of two
        // imaginary roots
        double t = -b / (2 * a);
        ...
        if (0 <= ratio_t && ratio_t <= 1) {
            ...
            // make sure ks[1] is in range [-pi, pi]
            ks[1] =
                (ks[0] > 0) ? (ks[0] - vnl_math::pi) : (ks[0] + vnl_math::pi);

            // need to find out which one gives min-distance
            double dist[2];
            for (int i = 0; i < 2; ++i) {
                vgl_point_2d<double> pt = arc.point_at_length(ks[i] / arc.k());
                dist[i] = vnl_math::hypot(pt.x() - line_foot.x(),
                                          pt.y() - line_foot.y());
            }
            double min_dist_ks = (dist[0] > dist[1]) ? ks[1] : ks[0];
...
// of the closest points
double dbgl_closest_point::lineseg_to_circular_arc(
    const vgl_line_segment_2d<double>& lineseg, const dbgl_circ_arc& arc,
...
// of the closest points
double dbgl_closest_point::circular_arc_to_circular_arc(
    const vgl_point_2d<double>& arc1_p1, const vgl_point_2d<double>& arc1_p2,
    double arc1_k, const vgl_point_2d<double>& arc2_p1,
    const vgl_point_2d<double>& arc2_p2, double arc2_k,
    vcl_vector<double>& arc1_ratios, vcl_vector<double>& arc2_ratios) {
    // See ultraserver.lems.brown.edu/vision/docs/kimia/reports/notes/
    // circular-arc-formulae/paper.tex for detailed description of algorithm
    ...
    if (vnl_math::abs(arc1_k) < dbgl_closest_point::min_curvature)
        return dbgl_closest_point::lineseg_to_circular_arc(
            arc1_p1, arc1_p2, arc2_p1, arc2_p2, arc2_k, arc1_ratios,
            arc2_ratios);

    if (vnl_math::abs(arc2_k) < dbgl_closest_point::min_curvature)
        return dbgl_closest_point::lineseg_to_circular_arc(
            arc2_p1, arc2_p2, arc1_p1, arc1_p2, arc1_k, arc2_ratios,
            arc1_ratios);
    ...
    // swap when |k2| < |k1| so that the term (k1/k2), involed in some
    // computation, is always in the range [-1, 1], This allows us avoiding
    // large number computation (when k1 >> k2)
    ...
    // from this point on, all codes should refer ONLY to arc1, arc1,
    // ret_ratios1, and ret_ratios2. DO NOT use arc1_p1, arc1_p2, ...
    ...
    if (roots.size() == 1 || roots.size() == 2) {
        for (unsigned int i = 0; i < roots.size(); ++i) {
            ...
            if (0 <= ratio_t && ratio_t <= 1) {
                vgl_vector_2d<double> psi0_plus_k1xs(
                    a * phi0_plus_k2xt.x() + b2, a * phi0_plus_k2xt.y() + b1);
    ...
    // if the two circles do NOT intersect, find their closest point
        ...
        k2xt[1] =
            (k2xt[0] > 0) ? k2xt[0] - vnl_math::pi : k2xt[0] + vnl_math::pi;
        ...
        k1xs[1] =
            (k1xs[0] > 0) ? (k1xs[0] - vnl_math::pi) : (k1xs[0] + vnl_math::pi);
    ...
    distances[0] = dbgl_closest_point::point_to_circular_arc(
        arc1.point1(), arc2.point1(), arc2.point2(), arc2.k(),
        arc2_candidate_ratios[0]);

    // 2nd set: arc1.point2() and arc2
    arc1_candidate_ratios[1] = 1;
    distances[1] = dbgl_closest_point::point_to_circular_arc(
        arc1.point2(), arc2.point1(), arc2.point2(), arc2.k(),
        arc2_candidate_ratios[1]);

    // 3rd set: arc2.point1() and arc1
    arc2_candidate_ratios[2] = 0;
    distances[2] = dbgl_closest_point::point_to_circular_arc(
        arc2.point1(), arc1.point1(), arc1.point2(), arc1.k(),
        arc1_candidate_ratios[2]);

    // 4th set: arc2.point2() and arc1
    arc2_candidate_ratios[3] = 1;
    distances[3] = dbgl_closest_point::point_to_circular_arc(
        arc2.point2(), arc1.point1(), arc1.point2(), arc1.k(),
        arc1_candidate_ratios[3]);
...
// at `ret_arc_length'
double dbgl_closest_point::point_to_biarc(const vgl_point_2d<double> query,
                                          const dbgl_biarc& biarc,
                                          double& ret_arclength) {
    ...
    if (biarc.len1() > dbgl_closest_point_distance_epsilon) {
        ...
        double ratio1 = -1;
        double d1 = dbgl_closest_point::point_to_circular_arc(
            query, arc1.point1(), arc1.point2(), arc1.k(), ratio1);
    ...
    if (biarc.len2() > dbgl_closest_point_distance_epsilon) {
        dbgl_circ_arc arc2(biarc.mid_pt(), biarc.point_at(biarc.len()),
                           biarc.k2());
        double ratio2 = -1;
        double d2 = dbgl_closest_point::point_to_circular_arc(
            query, arc2.point1(), arc2.point2(), arc2.k(), ratio2);
...
void dbgl_closest_point::solve_1st_order_trig_equation(
    double a, double b, double c, vcl_vector<vgl_vector_2d<double>>& roots) {
    roots.clear();
